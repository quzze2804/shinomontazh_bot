...
async def reschedule_booking(callback: types.CallbackQuery, state: FSMContext):
    ...
    if found_time:
        del bookings[found_time]
        await BookingStates.waiting_for_time.set()
        await callback.message.edit_text("🔁 Оберіть новий час для перенесення запису:", reply_markup=create_keyboard())
    else:
        await callback.message.edit_text("ℹ️ Активний запис не знайдено.")
